...
void CDBM::Clear()
{
    ...
    m_kBox.clear();
    m_kBox.shrink_to_fit();
    m_DataRows.clear();
    m_DataRows.shrink_to_fit();
    m_DataFieldNames.clear();
    m_DataFieldNames.shrink_to_fit();
...
CDBField& CDBM::GetField(const char* szName)
{
    ...
    if (this->m_kBox.empty() || !szName)  return emptyField;


    for (this->m_kBoxIterator = 0; this->m_kBoxIterator < this->m_kBox.size(); this->m_kBoxIterator++)
        if (strcmp(this->m_kBox[this->m_kBoxIterator].GetName(), szName) == 0)
