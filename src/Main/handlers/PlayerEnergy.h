    ...
    namespace Handlers
    {
        inline void PlayerEnergy(SCallbackData& callback, CMainServer* main_server)
        {
            ...
            BaseLib::DbPool->submit_task([server, main_server, unique_id]() mutable
            {
                ...
                auto temp_earnt_battery = acc_cache->earnt_battery;
                if (temp_earnt_battery + battery_earnt > 1000) return;
                if(temp_earnt_battery + acc_cache->acc_info.Energy > acc_cache->acc_info.MaximumEnergy) return;
                temp_earnt_battery += battery_earnt;
                if (BaseLib::Database->UpdateEnergy(static_cast<uint32_t>(acc_cache->acc_info.Energy + temp_earnt_battery), acc_cache->acc_info.AuthKey))
                {
					acc_cache->acc_info.Energy = static_cast<uint32_t>(acc_cache->acc_info.Energy + temp_earnt_battery);
					acc_cache->earnt_battery = temp_earnt_battery;
