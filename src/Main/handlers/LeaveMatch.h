        ...
        inline void LeaveMatch(SCallbackData& callback, CMainServer* main_server)
        {
            ...
            if (total_players_playing > 0)
            {
                ...
                else
                {
                    ...
                    acc_cache->playing = false;
                    /*
                    uint32_t my_team_id = static_cast<uint32_t>(acc_cache->team_id);
                    uint32_t my_team_left = 0;
                    //uint32_t my_team_left = (my_team_id == 0 ? (room_cache->neutralteam_session_ids.size() - 1) : (my_team_id == 1 ? room_cache->redteam_session_ids.size() : room_cache->blueteam_session_ids.size()));
