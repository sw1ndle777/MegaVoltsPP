...
namespace Game::Handlers
{
    ...
    inline void UsersProfile(SCallbackData& callback, CMainServer* main_server)
    {
        ...
        auto userInfoDetailsReq = reinterpret_cast<MainPlayerDetailsInfoReq*>(message->GetData());
        auto user_uniqueid = NetEngine::Packets::Core::UniqueId(userInfoDetailsReq->unique_id);
        auto target_acc_cache = CAccount.get<shared_t>(static_cast<uint16_t>(user_uniqueid.session));
        ...
        {
            if (CClan.contains(detailsInfo.ClanId))
            {
                auto clan_cache = CClan.get<shared_t>(detailsInfo.ClanId);
        ...
        auto setitem_info = CSetItemsInfo.get<shared_t>(set_item);
        auto assign_item = [&](int type, auto setitem_field)
            {
                ...
                DEBUGLOG(dark_cyan, "for type ({}) setitem_field is ({}) result is ({})", type, setitem_field, result);
                return result;
            };
        ...
        session->SendMsg(85, 0, detailsInfo.ClanId ? Userlist::Friends::DetailsType::WithClan : Userlist::Friends::DetailsType::WithoutClan, static_cast<uint8_t>(user_uniqueid.server), reinterpret_cast<uint8_t*>(&detailsInfo), sizeof(detailsInfo));
    }
}