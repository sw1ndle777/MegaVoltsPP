...
namespace Game::Handlers
{
    ...
    inline void Ping(SCallbackData& callback, CMainServer* main_server)
    {
        ...
        {
            DEBUGLOG(dark_cyan, "player session ({})'s ping couldn't update due to its account index being -1 or mission header being 1", session_id);
            return;
        }
        ...
        acc_cache.unlock();
        //DEBUGLOG(dark_cyan, "player ({}) has ping: ({})", acc_cache->acc_info.Nickname.c_str(), acc_cache->ping);

        if (!in_room || !CRoom.contains(room_id)) return;
