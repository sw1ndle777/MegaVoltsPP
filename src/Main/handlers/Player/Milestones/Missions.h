...
namespace Game::Handlers
{
    ...
    inline void Missions(SCallbackData& callback, CMainServer* main_server)
    {
        ...
        uint32_t guide_reward_mp = 0;
        uint32_t daily_reward_mp = 0;
        uint32_t daily_reward_item_id = 0;
        bool will_give_daily_item = false;
        bool will_give_daily_mp = false;
        ...
        if (is_complete_goal && is_guide_mission)
        {
            ...
            {
                guide_reward_mp = current_coll->rewardPoint;
                using enum CurrencyType;
                dctx.ops.emplace_back(AccountCurrencyDelta{ .type = MP, .value = guide_reward_mp, .is_reward = true });
            }
        }
        ...
        if ((is_complete_goal || is_reset_mission) && !is_guide_mission)
        {
            ...
            if (is_complete_goal)
            {
                ...
                if (completes)
                {
                    ...
                    {
                        daily_reward_mp = info->rewardPoint;
                        will_give_daily_mp = true;
                        using enum CurrencyType;
                        dctx.ops.emplace_back(AccountCurrencyDelta{ .type = MP, .value = daily_reward_mp, .is_reward = true });
                        DEBUGLOG(dark_cyan, "player get reward point: ({})", daily_reward_mp);
                    }
                    if (info->rewardItem)
                    {
                        daily_reward_item_id = info->rewardItem;
                        will_give_daily_item = true;
                        DEBUGLOG(dark_cyan, "player get reward item: ({})", daily_reward_item_id);
                        auto crafted_item = main_server->CraftInventoryItems(acc_cache, { daily_reward_item_id }, NetEngine::Items::Origin::From_Game);
            ...
            if (is_reset_mission)
            {
                if (acc_cache->acc_info.MicroPoints < 10000)
                {
        ...
        [[maybe_unused]] auto ignored = BaseLib::DbPool->submit_task([main_server, 
            ...
            mp_before = mp_before,
            guide_reward_mp = guide_reward_mp,
            daily_reward_mp = daily_reward_mp,
            daily_reward_item_id = daily_reward_item_id,
            will_give_daily_item = will_give_daily_item,
            will_give_daily_mp = will_give_daily_mp,
        ...
        ]() mutable
        {
            ...
            LogContext log_ctx;
            auto mp_after = new_acc_cache->acc_info.MicroPoints;
            if (is_complete_goal && is_guide_mission)
            {
                if (guide_reward_mp > 0)
                {
                    CurrencyLogEntry mp_log;
                    mp_log.aid = acc_index;
                    mp_log.currency_type = CurrencyLog::Type::MP;
                    mp_log.amount = static_cast<int32_t>(guide_reward_mp);
                    mp_log.before_value = mp_before;
                    mp_log.after_value = mp_after;
                    mp_log.source_type = CurrencyLog::SourceType::DailyMission;
            ...
            {
                if (level_up_result.reward_item.has_value())
                {
                    auto& reward_item = level_up_result.reward_item.value();
                    auto item_id = reward_item.item_number.item_id;
                    
                    ItemLogEntry item_log;
                    item_log.aid = acc_index;
                    item_log.action_type = ItemLog::ActionType::Added;
                    item_log.item_id = item_id;
                    item_log.serial_info = reward_item.serial_info.data;
            ...
            if (is_complete_goal && !is_guide_mission)
            {
                if (will_give_daily_mp)
                {
                    CurrencyLogEntry mp_log;
                    mp_log.aid = acc_index;
                    mp_log.currency_type = CurrencyLog::Type::MP;
                    mp_log.amount = static_cast<int32_t>(daily_reward_mp);
                    mp_log.before_value = mp_before;
                    mp_log.after_value = mp_after;
                    mp_log.source_type = CurrencyLog::SourceType::DailyMission;
                    log_ctx.currency_logs.push_back(mp_log);
                ...
                if (will_give_daily_item)
                {
                    for (const auto& added_item : v.items_added)
                    {
                        if (added_item.item_info.item_number.item_id == daily_reward_item_id)
                        {
                            ItemLogEntry item_log;
                            item_log.aid = acc_index;
                            item_log.action_type = ItemLog::ActionType::Added;
                            item_log.item_id = daily_reward_item_id;
                            item_log.serial_info = added_item.item_info.serial_info.data;
                            item_log.origin_type = ItemLog::OriginType::DailyMission;
                            
                            auto item_info = CItemsInfo.get<shared_t>(daily_reward_item_id);
                            if (item_info->Id)
                            {
                                item_log.item_type = static_cast<ItemLog::ItemType>(item_info->Type);
                            }
            ...
            if (!log_ctx.empty())
            {
                ...
                {
                    DEBUGLOG(red, "Failed to persist mission logs for player [{}]: {}", 
