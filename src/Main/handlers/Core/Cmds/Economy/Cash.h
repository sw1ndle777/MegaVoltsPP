...
namespace Game::Commands
{
    struct CashCommand
    {
        static constexpr std::string_view name = "cash";
        ...
        static void Run(std::span<const std::string_view> args, CommandContext& ctx)
        {
            ...
            {
                ctx.server->SendServerMessage(ctx.callback.session, "[MegaVolts Online] usage: /cash <amount> [nickname]");
                return;
            }
            ...
            auto current = target_acc->acc_info.RockTokens;
            auto desired = *amount;
            if (current == desired)
            {
                ctx.server->SendServerMessage(ctx.callback.session, fmt::format("[MegaVolts Online] RT is already {}", desired));
                return;
            }
            ...
            if (desired > current)
                dctx.ops.emplace_back(AccountCurrencyDelta{ .type = RT, .value = desired - current, .is_reward = true });
            else
                dctx.ops.emplace_back(AccountCurrencyDelta{ .type = RT, .value = current - desired, .is_reward = false });

            auto validated = ctx.server->ValidateDatabaseUpdates(target_acc, dctx, false, true);
            if (!validated.has_value())
            {
                ctx.server->SendServerMessage(ctx.callback.session, "[MegaVolts Online] failed to update RT");
                DEBUGLOG(red, "ValidateDatabaseUpdates failed for cash cmd: {}", static_cast<int>(validated.error()));
                return;
            }

            auto rt_before = current;
            ...
            ]() mutable
            {
                ...
                log.aid = t_aid;
                log.currency_type = CurrencyLog::Type::RT;
                log.amount = static_cast<int32_t>(desired) - static_cast<int32_t>(rt_before);
                log.before_value = rt_before;
                log.after_value = new_acc->acc_info.RockTokens;
                ...
                if (auto target_session = server->GetSessionById(t_sid))
                    SendCurrencyPacket(target_session.get(), new_acc);

                server->SendServerMessage(session, fmt::format("[MegaVolts Online] set RT to {} for {}", new_acc->acc_info.RockTokens, t_nick));
            });
        }

        inline static CommandRegister<CashCommand> reg{};
    };
}
