...
namespace Game::Commands
{
    struct PointCommand
    {
        ...
        static void Run(std::span<const std::string_view> args, CommandContext& ctx)
        {
            ...
            {
                ctx.server->SendServerMessage(ctx.callback.session, "[MegaVolts Online] usage: /point <amount> [nickname]");
                return;
            }
            ...
            auto current = target_acc->acc_info.MicroPoints;
            auto desired = *amount;
            if (current == desired)
            {
                ctx.server->SendServerMessage(ctx.callback.session, fmt::format("[MegaVolts Online] MP is already {}", desired));
                return;
            }
            ...
            if (desired > current)
                dctx.ops.emplace_back(AccountCurrencyDelta{ .type = MP, .value = desired - current, .is_reward = true });
            else
                dctx.ops.emplace_back(AccountCurrencyDelta{ .type = MP, .value = current - desired, .is_reward = false });

            auto validated = ctx.server->ValidateDatabaseUpdates(target_acc, dctx, false, true);
            if (!validated.has_value())
            {
                ctx.server->SendServerMessage(ctx.callback.session, "[MegaVolts Online] failed to update MP");
                DEBUGLOG(red, "ValidateDatabaseUpdates failed for point cmd: {}", static_cast<int>(validated.error()));
                return;
            }

            auto mp_before = current;
            ...
            ]() mutable
            {
                ...
                log.aid = t_aid;
                log.currency_type = CurrencyLog::Type::MP;
                log.amount = static_cast<int32_t>(desired) - static_cast<int32_t>(mp_before);
                log.before_value = mp_before;
                log.after_value = new_acc->acc_info.MicroPoints;
                ...
                server->SendServerMessage(session, fmt::format("[MegaVolts Online] set MP to {} for {}", new_acc->acc_info.MicroPoints, t_nick));
            });
        }

        inline static CommandRegister<PointCommand> reg{};
    };
}
